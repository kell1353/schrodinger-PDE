...
implicit none
private
public :: solve_linear_system, test_array_sizes, invert_matrix, matrix_mult, mult_matrix_vector
contains
...
!!
!! b_vector         real        1D array containing the $b$ vector
!! a                real        2D array with the matrix 
!-----------------------------------------------------------------------
!!----------------------------------------------------------------------
!! Output:
!!
...
!-----------------------------------------------------------------------
subroutine mult_matrix_vector(b_vector, a, x_vec)
    implicit none
    real(dp), intent(in) :: b_vector(:), a(:,:)
    real(dp), intent(out) :: x_vec(:)

    integer :: i, j, n, shape_ainv(1:2)
    ...
    do i=1,n
        do j=1,n
            x_vec(i) = x_vec(i) + (a(i,j)*b_vector(j))
        enddo
    enddo
...
!! Input:
!!
!! a        real   2D array with the first matrix 
!! b        real   2D array with the second matrix
!-----------------------------------------------------------------------
!!----------------------------------------------------------------------
!! Output:
!!
!! array_out   real    1D array containing the results of the multiplication
!-----------------------------------------------------------------------
subroutine matrix_mult(a, b, array_out)
    implicit none
    real(dp), intent(in) :: a(:,:), b(:,:)
    real(dp), intent(out) :: array_out(:,:)

    real(dp) :: tmp
    integer :: i, j, k, n, shape_ainv(1:2)
